...
> **중요**: 상품 폴더명 앞에 CSV 입력 순서에 따른 순번(001, 002, ...)이 붙습니다.
> 탐색기에서 **이름순 정렬**하면 `messages.txt` 내 상품 순서와 폴더 순서가 일치하므로,
> 메시지 전송 시 해당 순번의 폴더에서 이미지를 함께 업로드하면 됩니다.
...
| `messages.txt` | 모든 상품의 메시지를 순번 구분선(`==== [001]`)으로 합친 파일 |
