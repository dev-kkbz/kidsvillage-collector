...
from src.resource import application_dir
...
class App(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Entry(input_frame, textvariable=self._var_username, width=30).grid(row=0, column=1, columnspan=2, sticky="w", **pad)

        ttk.Label(input_frame, text="비밀번호:").grid(row=1, column=0, sticky="e", **pad)
        self._var_password = tk.StringVar()
        ttk.Entry(input_frame, textvariable=self._var_password, width=30, show="*").grid(row=1, column=1, columnspan=2, sticky="w", **pad)

        ttk.Label(input_frame, text="CSV 파일:").grid(row=2, column=0, sticky="e", **pad)
        self._var_csv = tk.StringVar()
        ttk.Entry(input_frame, textvariable=self._var_csv, width=30).grid(row=2, column=1, sticky="w", **pad)
        ttk.Button(input_frame, text="찾아보기", command=self._browse_csv).grid(row=2, column=2, **pad)

        ttk.Label(input_frame, text="출력 폴더:").grid(row=3, column=0, sticky="e", **pad)
        self._var_output = tk.StringVar()
        ttk.Entry(input_frame, textvariable=self._var_output, width=30).grid(row=3, column=1, sticky="w", **pad)
        ttk.Button(input_frame, text="찾아보기", command=self._browse_output).grid(row=3, column=2, **pad)
        ...
        self._log_text = tk.Text(log_frame, height=15, width=70, state="disabled", wrap="word")
    ...
    def _setup_logging(self) -> None:
        ...
        file_handler = logging.FileHandler(log_dir / "debug.log", mode="w", encoding="utf-8")
    ...
    def _load_credentials(self) -> None:
        path = application_dir() / self.CREDENTIALS_FILE
        if not path.exists():
            return
        try:
            data = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
            if data.get("username"):
                self._var_username.set(str(data["username"]))
            if data.get("password"):
                self._var_password.set(str(data["password"]))
    ...
    def _browse_output(self) -> None:
        path = filedialog.askdirectory(title="출력 폴더 선택", initialdir=application_dir())
        if path:
            self._var_output.set(path)

    def _open_output(self) -> None:
        output_text = self._var_output.get().strip()
        if not output_text:
            messagebox.showwarning("경고", "출력 폴더를 먼저 선택해주세요.")
            return
    ...
    def _run_pipeline(self, username: str, password: str, csv_path: str, output_dir: str) -> None:
        try:
            config = load_config()
            ...
            orchestrator = ProductOrchestrator(config, on_progress=self._on_progress)
    ...
    def _on_progress(self, current: int, total: int, product_id: str) -> None:
        self.after(0, self._apply_progress, current, total, product_id)

    def _apply_progress(self, current: int, total: int, product_id: str) -> None:
    ...
    def _finish(self, message: str) -> None:
        self.after(0, self._apply_finish, message)

    def _apply_finish(self, message: str) -> None:
